...
void MessageHandler::handleCameraMessage(const std::string& topic, const std::string& payload) {
    ...
    for (auto& [category, detections] : message->objects) {
        ...
        // Propagate observability context to each batch
        ObservabilityContext batch_ctx = obs_ctx;
        batch_ctx.category = category;
        ...
        batch.obs_ctx = std::move(batch_ctx);
    ...
    obs_ctx.buffer_time = std::chrono::steady_clock::now();
