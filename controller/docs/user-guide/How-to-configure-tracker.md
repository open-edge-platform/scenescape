...
- `time_chunking_rate_fps`: Defines the tracker processing rate in frames per second (valid range: 1–250). The tracker processes data in chunks at intervals of `1000 / time_chunking_rate_fps` milliseconds. For example, if `time_chunking_rate_fps` is 15, the time chunking interval is approximately 67 ms.
