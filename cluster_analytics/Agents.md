...
Before running test targets from `tests/`, initialize the required test images and secrets in a fresh environment with `SUPASS=<password> make setup_tests`. If your environment requires a proxy, rely on your existing `HTTP_PROXY`/`HTTPS_PROXY` settings instead of hard-coding proxy values in the command line.

| Change class                  | Command path                                                                                              | Pass criteria                                                                                |
| ----------------------------- | --------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| API/schema contracts          | `make cluster_analytics && make -C cluster_analytics test-build && make -C tests unit-tests`             | Exit code 0; no new publish-contract/schema regressions in test output.                      |
| Algorithm/lifecycle logic     | `make cluster_analytics && make -C cluster_analytics test-build && make -C tests unit-tests`             | Exit code 0; no regressions in stability-oriented checks and scene processing behavior.      |
| Performance-sensitive changes | `make -C tests metrics`                                                                                   | Exit code 0; include before/after latency and cluster ID churn measurements for tuned logic. |
