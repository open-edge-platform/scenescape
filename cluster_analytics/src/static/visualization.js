...
const clusterColors = [
    ...
    '#fd79a8', '#00b894', '#0984e3', '#fdcb6e', '#a29bfe'
];
...
function updateUI() {
    // Update stats
    document.getElementById('objectCount').textContent = sceneData.objects?.length || 0;
    document.getElementById('clusterCount').textContent = sceneData.clusters?.length || 0;
    
    if (sceneData.metadata?.timestamp) {
        const time = new Date(sceneData.metadata.timestamp * 1000).toLocaleTimeString();
