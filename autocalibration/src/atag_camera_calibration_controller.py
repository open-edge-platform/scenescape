  ...
  def saveToDatabase(self, scene, atag_points_3d):
    ...
    if apriltags and len(atag_points_3d) < len(apriltags):
      ...
      for key, value in atag_points_3d.items():
        post_data = {'marker_id': f"{scene.id}_{
            str(key)}", 'apriltag_id': key, 'dims': value, 'scene': scene.id}
  ...
  def generateCalibration(self, sceneobj, camera_intrinsics, cam_frame_data):
    ...
    try:
      ...
      if (cur_cam_calib_obj.result_data_3d is None
        ...
        raise TypeError((
            f"Fewer than {MIN_APRILTAG_COUNT} tags found in {
                sceneobj.name}'s map. Make sure "
      ...
      log.info(f"Image decoded for camera {cam_frame_data['id']}, shape: {
               src_2d_image.shape if src_2d_image is not None else 'None'}")
      ...
      if (camera_pose is not None
        ...
        if (cam_frame_data['id'] not in self.frame_count or
          ...
          raise TypeError((
              f"Fewer than {MIN_APRILTAG_COUNT} tags found in {
                  cam_frame_data['id']}'s"
              f"feed. Make sure there are at least {
                  MIN_APRILTAG_COUNT} tags clearly "
