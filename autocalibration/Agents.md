...
Note: `auto-calibration` and `markerless-unit` tests start docker-compose-backed test stacks. Prepare test images and secrets first, for example: `SUPASS=<password> make setup_tests`. If your environment requires a proxy, use your existing `HTTP_PROXY`/`HTTPS_PROXY` environment variables rather than hard-coding proxy values in these commands.

| Change class                          | Command path                                                                                             | Pass criteria                                                                                      |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| API/schema contracts                  | `make autocalibration && make -C autocalibration test-build && make -C tests auto-calibration`          | Exit code 0; auto-calibration workflow test passes without new contract/schema failures.           |
| Algorithm/concurrency flow            | `make autocalibration && make -C autocalibration test-build && make -C tests markerless-unit`           | Exit code 0; markerless calibration path passes with no new race/locking regressions.              |
| Performance/quality-sensitive changes | `make -C tests auto-calibration`                                                                         | Exit code 0; report before/after p95 completion and reprojection-quality deltas for changed logic. |
