...
- Test files: `test_*.py`
- Test classes: `Test*` (e.g., `TestSceneController`)
- Test functions: `test_*` (e.g., `test_validation`)
...
def sample_scene():
    """Fixture providing a sample scene object."""
...
class TestSceneViews(TestCase):
    def setUp(self):
        self.scene = Scene.objects.create(name="test")

    def test_scene_detail(self):
        response = self.client.get(f'/scene/{self.scene.id}/')
        self.assertEqual(response.status_code, 200)
```

### Zephyr Test IDs

All tests must include NEX-T##### ID:
