...
- Client libraries live **under `scene_common/src`** (decision locked), one subpackage per OEP
  component, owning transport/auth/certs, typed API, schema validation, bounded retries +
  backoff, OTEL spans, version negotiation, and shipped test doubles (per design §5.3, §5.4.1).
...
1. **Client-library location:** under `scene_common/src` (shared, single dependency reused across services).
...
| S6 | Shared demo Docker Compose integration | Story | S1, S2 |
...
**Location.** New subpackage `scene_common/src/scene_common/integration/vippet/`.
...
4. Add OTEL spans/metrics named `vippet.*` (e.g. `vippet.get_pipeline_definition`), aligned with
   `controller/observability/` conventions.
5. Ship a fake/mock (in-package test double) that serves canned pipeline lists/definitions.

**Files (new).** `scene_common/src/scene_common/integration/vippet/{__init__.py,client.py,models.py,schema.py,fakes.py}`; schema JSON under the same package; tests under `tests/sscape_tests/integration/vippet/`.
...
**Location.** New subpackage `scene_common/src/scene_common/integration/dlsps/`.
...
4. Add OTEL spans/metrics named `dlsps.*`.
5. Ship a stateful fake that tracks created/started/stopped instances for downstream tests.

**Files (new).** `scene_common/src/scene_common/integration/dlsps/{__init__.py,client.py,models.py,fakes.py}`; tests under `tests/sscape_tests/integration/dlsps/`.
...
1. Add a Django model in `manager/src/manager/` for the pipeline-to-camera mapping: FK to scene,
   list of camera bindings (one-to-many), embedded pipeline-definition body (by value), and
   per-camera parameter values (source ID, confidence threshold, NTP usage, source address —
   design §5.5.2 / §5.7).
2. Add the DB migration (follow `manager/MIGRATIONS.md`).
3. Extend the Manager REST API with CRUD endpoints for the mapping (server-side authorization).
4. Ensure legacy scenes with no mapping are unaffected (nullable/absent mapping = legacy flow).

**Files.** New model + migration + serializer/view under `manager/src/manager/`; tests under
...
per-camera params round-trip; legacy scene (no mapping) unaffected.

**AC.** Mapping persists scene-side; definition stored by value; legacy scenes unaffected; unit-tested.

### S4 — Manager UI: load pipelines, map to cameras, lifecycle control + monitor

**Goal.** New additive UI surface to load the ViPPET pipeline list (S1), assign a pipeline to each
scene camera (S3), and start/stop/monitor them (S2 via S5). **No parameter editing in-UI** (out of MVP scope).

**Steps.**

1. Add a UI surface (template + JS) in `manager/src/` to list ViPPET pipelines via the S1-backed backend endpoint.
2. Assign a pipeline to each scene camera and persist via the S3 mapping API.
3. Start/stop controls and a running-status/monitor view wired to the S5 orchestration endpoints.
4. Keep the legacy UI flow untouched and reachable; the new surface is purely additive.

**Files.** Templates + static JS under `manager/src/manager/` (follow the JavaScript skill);
new views/routes; UI tests under `tests/ui/` where applicable.
...
running status; legacy UI flow untouched.
...
**Location.** New subpackage `scene_common/src/scene_common/integration/stream_manager/`.
...
4. Ship a fake for tests.

**Files.** `scene_common/src/scene_common/integration/stream_manager/{...}`; consumer in `manager/src/`; tests.
...
**S9** anytime after S1 patterns are established (stretch).
...
- **Observability:** per-component OTEL spans/metrics (latency, error rate, retry count) inside each client library.
