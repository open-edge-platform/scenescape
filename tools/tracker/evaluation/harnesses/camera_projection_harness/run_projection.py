...
def _apply_size_offset(
  wx: float, wy: float,
  cam_tx: float, cam_ty: float,
  x_size: float, y_size: float,
) -> tuple[float, float]:
  """Push world position away from camera by mean([x_size, y_size]) / 2.

  Replicates the size-offset step in
  ``MovingObject.mapObjectDetectionToWorld()`` (controller/moving_object.py).

  Returns unchanged position when both sizes are zero or the distance is
  degenerate.
  """
  offset = (x_size + y_size) / 4.0   # mean([x_size, y_size]) / 2
  if offset < 1e-9:
    return wx, wy
  dx, dy = wx - cam_tx, wy - cam_ty
  dist = math.sqrt(dx * dx + dy * dy)
  if dist < 1e-9:
    return wx, wy
  return (wx + (dx / dist) * offset, wy + (dy / dist) * offset)
...
def project_frame(
  ...
  for category, obj_list in detection_frame.get("objects", {}).items():
    ...
    for obj in obj_list:
      ...
      # Camloc compensation: push object away from camera by size offset
      wx, wy = _apply_size_offset(
