...
- **BlackBoxHarness**: `harnesses/black_box_harness/black_box_harness.py`
  ...
  - Also copies the tracker configuration file into `<output_folder>/config/` (both container types), preserving the original filename, so each run captures the config it used.
