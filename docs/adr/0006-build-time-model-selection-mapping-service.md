...
One of the primary challenges in configuring SceneScape for a new scene is the availability of map and the subsequent calibration of cameras against that map. Previous methods for configuring a map involved obtained floor blueprints or meshes reconstructed using third-party software. Camera calibration involved manual calibration, scene augmentation with apriltags or dependency on a specify third-party reconstruction software's output format. This results in barriers to evaluation, adoption and deployments at scale without significant labor cost to the user. Additionally, the same costs need to be paid when cameras are moved (perform recalibration) or if the scene changes (regenerate mesh).
