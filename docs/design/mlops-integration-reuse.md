...
4. **Model artifact support.** The scene artifact format supports both model artifact variants described in the _Package structure_: model identifiers embedded in pipeline definitions (standard path) and an optional compressed model volume archive included in the deployment package (air-gap / offline path). When the archive is present, the deployment operator's extraction job or script extracts it at deployment time, without requiring Model Downloader or network access.
