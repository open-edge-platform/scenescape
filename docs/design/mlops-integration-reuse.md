...
| Payload                   | A pipeline definition containing: a DLSPS-consumable pipeline definition body, parametrized model reference(s) by ID + version, and pipeline metadata. **Models are parameters of the pipeline definition; they are referenced by identifier (path) — not embedded.** |
