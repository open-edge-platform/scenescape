...
**child** broker. The child enables `DATA_EXTERNAL` for that scene automatically and
disables it again on parent disconnect (`detached`). Local children on the same
controller do not need this signal — they already have `scene.parent` set. See
