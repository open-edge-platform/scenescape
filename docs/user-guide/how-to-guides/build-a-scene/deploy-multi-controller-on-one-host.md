...
The child Scene Controller enables `DATA_EXTERNAL` for that scene while the parent is
linked and disables it on disconnect (`detached`). Local children on the same
controller do not need this signal.
