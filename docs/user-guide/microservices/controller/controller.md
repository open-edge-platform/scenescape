...
`detached`). The child enables `DATA_EXTERNAL` for that scene while attached and disables it on
disconnect. Local children (same controller, `scene.parent` set) do not use this signal.
