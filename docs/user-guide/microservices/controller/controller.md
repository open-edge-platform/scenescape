...
`--person-pose-adjustment`: Enables pose-based person bounding box adjustment before world projection. When enabled, the controller uses pose keypoints (e.g. from a `yolo11n-pose` model) to refine the bounding box used for projecting person detections into world coordinates. This is disabled by default. Can also be enabled via the `CONTROLLER_ENABLE_PERSON_POSE_ADJUSTMENT` environment variable set to `true`. Requires the DL Streamer video pipeline to use a pose estimation model that provides keypoint data. See the [DL Streamer Pipeline Server documentation](../../../dlstreamer-pipeline-server/README.md#enable-pose-estimation) for pipeline setup.
