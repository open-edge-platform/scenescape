...
| `camera_bounds`      | object             | Per-camera pixel bounding boxes (`{camera_id: {x, y, width, height, projected}}`) where `projected=false` means detector-provided pixel bbox and `projected=true` means computed projection (including in analytics-only mode when the tracked object includes a `size` field); may be empty (`{}`) when no camera currently observes the track |
