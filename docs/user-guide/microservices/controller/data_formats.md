...
- **Configured child scene** (no `source_id`): `{publisher_id}` is the sending child's
  ...
  Only scenes with a parent, or controllers with `--publish-external` /
  `CONTROLLER_PUBLISH_EXTERNAL` enabled (remote-child opt-in), publish this hierarchy
  form; standalone roots do not emit hierarchy echoes onto the external topic.
