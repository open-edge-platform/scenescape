...
- **`OVERRIDE_autodelete_interval_s`** (VDMS server-side, Docker env var) — how often, in seconds, the VDMS server sweeps for and removes expired descriptors. This does not change *when* a descriptor expires, only how promptly expired descriptors are cleaned up after the fact. Set on the `vdms` container, e.g.:
