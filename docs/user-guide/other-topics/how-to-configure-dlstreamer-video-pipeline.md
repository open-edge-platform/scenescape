...
- **camera_config**: Configuration for the camera and its metadata publishing.
  ...
  - **metadatagenpolicy** (string): Policy for generating metadata. Possible values:
    - `detectionPolicy` (default): Metadata for object detection. When a pose estimation model is used (e.g. `yolo11n-pose`), also includes `keypoints` (named joints with normalized x/y coordinates) and `keypoint_connections` (skeleton bone pairs) in each detection object.
