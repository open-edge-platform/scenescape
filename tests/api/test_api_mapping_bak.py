...
def execute_step(step, step_number, total_steps):
  ...
  # Handle new structured format with path_params and body
  # New format:
  #   "request": {
  #     "path_params": {"scene_id": "123", "uid": "456"},
  #     "body": {"name": "Test"}
  #   }
  # Old format (backward compatible):
  #   "request": {"scene_id": "123", "body": {"name": "Test"}}
  
  method_args = {}
  
  if "path_params" in request_data or "body" in request_data:
    # New structured format
    # Extract path parameters
    if "path_params" in request_data:
      method_args.update(request_data["path_params"])
    
    # Extract body and map to 'data' parameter
    if "body" in request_data:
      method_args["data"] = request_data["body"]
    
    # Extract other parameters (like filter, query params, etc.)
    for key in request_data:
      if key not in ["path_params", "body"]:
        method_args[key] = request_data[key]
  else:
    # Old format - backward compatible
    # Map old keys to method parameter names:
    #   "UID"  -> "uid"   (path parameter)
    #   "body" -> "data"  (request body)
    KEY_MAP = {"UID": "uid", "body": "data"}
    method_args = {KEY_MAP.get(k, k): v for k, v in request_data.items()}
  ...
  if "filter" in method_params and "filter" not in method_args:
    method_args["filter"] = None

  # Log the method call with ALL arguments (using method_args, not request_data)
  def format_arg(k, v):
    if isinstance(v, dict):
      return f"{k}={{...}}"  # Show dict exists but don't expand it
    elif isinstance(v, (list, tuple)) and len(v) > 3:
      return f"{k}=[{len(v)} items]"  # Show list length
    else:
      v_str = repr(v)
      return f"{k}={v_str[:100]}" if len(v_str) > 100 else f"{k}={v_str}"
  
  args_preview = ', '.join(format_arg(k, v) for k, v in method_args.items())
  logger.info(f"    Calling: {api_name}.{method_name}({args_preview})")

  poll_config = step.get("poll")

  # Execute API call — with optional polling loop
  try:
    if poll_config:
      ...
      response = None
      while True:
        response = api_method(**method_args)
  ...
  # Check status code - support single int or list of acceptable codes
