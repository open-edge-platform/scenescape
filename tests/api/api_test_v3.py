...
BASE_URL = os.environ.get("API_BASE_URL", "https://localhost/v1")
...
def build_call_kwargs(request_data):
  ...
  Supported top-level keys in request_data:
    ...
    filter       – forwarded unchanged.
    Any other key is forwarded unchanged (backward-compat with old flat style).

  Example input:
    {
      "path_params": {"scene_id": "abc123"},
      "body": {"calibration_mode": "auto"}
    }
  Example output:
    {"scene_id": "abc123", "data": {"calibration_mode": "auto"}}
  """
  kwargs = {}
...
def execute_step(step, step_number, total_steps):
  ...
  # Log the method call exactly as it will be invoked
  args_preview = ', '.join(
      f"{k}={repr(v)[:200]}{'...' if len(repr(v)) > 200 else ''}"
      for k, v in call_kwargs.items()
  )
  logger.info(f"    Calling: {api_name}.{method_name}({args_preview})")
  ...
  # Log the actual HTTP request that was made.
  # RESTClient may not attach a PreparedRequest, so we try three sources:
  #   1. response.request  (requests.PreparedRequest – most complete)
  #   2. response.url      (requests.Response always sets this directly)
  #   3. Infer method from the method name prefix as a last resort
  _method = None
  _url = None

  if hasattr(response, 'request') and response.request is not None:
    logger.info(f"    HTTP Request: {response.request.method} {response.request.url}")
    _method = response.request.method
    _url = response.request.url
  elif getattr(response, 'url', None):
    _url = response.url
    # Derive a best-guess HTTP verb from the API method name
    _name_lower = method_name.lower()
    if _name_lower.startswith(("create", "register", "add", "post", "calibrate")):
      _method = "POST"
    elif _name_lower.startswith(("update", "patch")):
      _method = "PATCH"
    elif _name_lower.startswith(("put", "replace", "set")):
      _method = "PUT"
    elif _name_lower.startswith(("delete", "remove")):
      _method = "DELETE"
    else:
      _method = "GET"

  if _url:
    logger.info(f"    → {_method or '???'} {_url}")
  else:
    logger.warning("    → HTTP request details unavailable")
  ...
  # Check status code
  expected_status_code = expected_status.get("status_code", 200)
  if response.status_code != expected_status_code:
    return False, response, f"Expected status {expected_status_code}, got {response.status_code}"
