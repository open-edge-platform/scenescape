...
def load_scenarios(path=None):
    ...
    if os.path.isfile(path):
        ...
        logger.info(f"Loading {len(scenario_files)} scenario files from folder: {path}")
...
def pytest_generate_tests(metafunc):
    ...
    if "test_case" in metafunc.fixturenames:
        ...
        # filter by test_case ID if specified (e.g., --test_case Vision_AI/SSCAPE/API/SCENE/01)
        if test_case_filter:
            original_count = len(scenarios)
            scenarios = [s for s in scenarios if s.get("test_name", "").split(":")[0].strip() == test_case_filter]
            logger.info(f"Filtered scenarios: {len(scenarios)}/{original_count} matching test_case '{test_case_filter}'")
