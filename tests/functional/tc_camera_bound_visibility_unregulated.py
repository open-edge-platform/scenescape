...
class CameraBoundVisibility(CameraBounds):
    def check_camera_bound_visibility(self):
        ...
        if self.visibility_topic == "unregulated":
            raise AssertionError("Expected camera_bounds in BOTH regulated and unregulated topics")

@pytest.mark.parametrize("test_name", ["NEX-T19788"])
def test_camera_bound_visibility(params, pytestconfig, record_xml_attribute, test_name):
