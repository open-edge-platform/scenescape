...
def make_detection(det_id, bbox, embedding=None,
...
def get_scene_and_camera(params):
    ...
    for scene in results:
        ...
        if cam_results:
            ...
            log.info(
                f"Using scene={
                    scene['name']} ({scene_uid}) camera={camera_id}")
...
def test_persist_stored_to_vdms_on_track_end(scenescape_env, params, mqtt_client,
    ...
    with SceneOutputCollector(mqtt_client, scene_uid) as collector:
        publish_frames(
            ...
            camera_id,
            [det],
            num_frames=FEATURE_THRESHOLD +
            8)
        tracked = collector.wait_for(
            lambda o: (
        ...
        obj = collector.wait_for(
            lambda o: gender_label(o) == "Male", timeout=20)
    ...
    log.info(f"VDMS FindDescriptor for gid={
             gid} returned {len(entities)} entities")
    assert entities, f"No reid_vector descriptor for gid={gid}"
    persist = entities[0].get("persist")
    assert isinstance(
        persist, dict) and "gender" in persist, f"persist missing gender: {persist}"
    assert persist["gender"].get("label") == "Male", f"persist gender label mismatch: {
        persist.get('gender')}"

    if log_contains(
            scenescape_env, r"_addNewFeaturesToDatabase: Adding \d+ features", timeout=5.0):
        log.info("_addNewFeaturesToDatabase log line observed")
    else:
        log.info(
            "_addNewFeaturesToDatabase log line not observed (requires DEBUG log level)")
...
def test_gender_survives_intermittent_dropouts(mqtt_client,
    ...
    det_present = make_detection(
        3,
        GENDER_BBOX,
        embedding=emb,
        gender="Male",
        gender_conf=0.9)
    det_dropout = make_detection(
        3,
        GENDER_BBOX,
        embedding=emb,
        gender="Male",
    ...
    with SceneOutputCollector(mqtt_client, scene_uid) as collector:
        # Warm-up: establish persistent_data.gender == "Male" before
...
def test_high_confidence_gender_used_as_tier1_constraint(scenescape_env, mqtt_client,
    ...
    assert reentry_gid == gid, \
        f"re-entered track (gid={reentry_gid}) did not match baseline gid={
            gid} via VDMS ReID"
    ...
    if log_contains(scenescape_env,
        ...
        log.info(
            "TIER-1 gender constraint log line not observed (requires DEBUG log level)")

    det_negative = make_detection(
        8,
        REENTRY_BBOX,
        embedding=emb,
        gender="Female",
    ...
    with SceneOutputCollector(mqtt_client, scene_uid) as collector:
        publish_frames(
            ...
            [det_negative],
            num_frames=FEATURE_THRESHOLD + 8)
        negative_obj = collector.wait_for(
            lambda o: gender_label(o) == "Female", timeout=20)
