...
# Legacy declaration: TEST_NAME assigned inside the test body. Indentation is
# what distinguishes it from the module-level constant.
LOCAL_TEST_NAME_RE = re.compile(r"^[ \t]+TEST_NAME\s*=\s*['\"]([^'\"]+)['\"]", re.M)
...
def _local_test_name(item):
  """Return a TEST_NAME assigned in the test body, which is invisible at collection."""
  ...
  try:
    ...
    return None
  match = LOCAL_TEST_NAME_RE.search(source)
...
def pytest_collection_finish(session):
  ...
  for item in session.items:
    ...
    local_name = _local_test_name(item)
    records.append({
      "nodeid": item.nodeid,
      ...
      # Legacy declaration styles, reported to guide migration to the marker.
      "module_test_name": str(module_name) if module_name is not None else None,
      "local_test_name": local_name,
    })
