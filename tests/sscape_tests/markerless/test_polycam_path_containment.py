...
def makeScene(name, polycam_data=None):
  scene = CalibrationScene(uid="f1b9b1b0-1b1b-1b1b-1b1b-1b1b1b1b1b1b", name=name)
...
def test_is_within_datasets_root_allows_safe_name(controller, tmp_path, monkeypatch):
  ...
  monkeypatch.chdir(tmp_path)
  assert controller._is_within_datasets_root(tmp_path / "datasets" / "my-scene")
  assert controller._is_within_datasets_root(tmp_path / "datasets" / "sub" / "dir")
...
def test_is_within_datasets_root_rejects_escape(controller, tmp_path, monkeypatch):
  ...
  monkeypatch.chdir(tmp_path)
  assert not controller._is_within_datasets_root(tmp_path / "datasets" / ".." / ".." / "etc")
