...
class TestMetricFootprintArea:
  """Test _metricFootprintArea helper for objects without a pixel bounding box."""

  def test_returns_none_when_size_attribute_missing(self, mock_vdms_db):
    """Verify None is returned when sscape_object has no size attribute at all."""

    manager = UUIDManager()

    obj = Mock(spec=['rv_id'])
    obj.rv_id = "track_no_size"

    area = manager._metricFootprintArea(obj)

    assert area is None, "Should return None when size attribute is missing"

  def test_returns_none_when_size_is_none(self, mock_vdms_db):
    """Verify None is returned when size is explicitly None."""
    ...
    assert area is None, "Should return None when size is None"

  def test_computes_area_from_valid_size(self, mock_vdms_db):
    """Verify width_m * height_m is computed correctly from a well-formed size list."""

    manager = UUIDManager()

    obj = MagicMock()
    obj.size = [0.5, 0.5, 1.7]  # [width_m, width_m, height_m]

    area = manager._metricFootprintArea(obj)

    assert area == pytest.approx(0.85), "Should compute width * height as 0.5 * 1.7"

  def test_returns_none_when_size_too_short(self, mock_vdms_db):
    """Verify None is returned when size doesn't have enough elements to index [2]."""

    manager = UUIDManager()

    obj = MagicMock()
    obj.size = [0.5, 0.5]  # missing height at index 2

    area = manager._metricFootprintArea(obj)

    assert area is None, "Should return None when size is too short"

  def test_returns_none_when_size_contains_non_numeric_values(self, mock_vdms_db):
    """Verify None is returned when size elements can't be converted to float."""

    manager = UUIDManager()

    obj = MagicMock()
    obj.size = ["not_a_number", 0.5, "also_not_a_number"]

    area = manager._metricFootprintArea(obj)

    assert area is None, "Should return None when size elements are non-numeric"

  def test_returns_none_when_width_is_nan(self, mock_vdms_db):
    """Verify None is returned when width is NaN."""

    manager = UUIDManager()

    obj = MagicMock()
    obj.size = [float('nan'), float('nan'), 1.7]

    area = manager._metricFootprintArea(obj)

    assert area is None, "Should return None when width is NaN"

  def test_returns_none_when_height_is_infinite(self, mock_vdms_db):
    """Verify None is returned when height is infinite."""

    manager = UUIDManager()

    obj = MagicMock()
    obj.size = [0.5, 0.5, float('inf')]

    area = manager._metricFootprintArea(obj)

    assert area is None, "Should return None when height is infinite"

  def test_handles_zero_size_values(self, mock_vdms_db):
    """Verify zero-valued dimensions produce a zero area rather than an error."""

    manager = UUIDManager()

    obj = MagicMock()
    obj.size = [0.0, 0.0, 0.0]

    area = manager._metricFootprintArea(obj)

    assert area == 0.0, "Should compute zero area without raising"

class TestGatherQualityVisualFeaturesMetricFallback:
  """Test gatherQualityVisualFeatures metric-space fallback for objects without a pixel bbox.

  Covers the parent/child-hierarchy case: objects reconstructed on a parent scene from
  data forwarded by a remote child never have boundingBoxPixels set (see MovingObject and
  detections_builder.prepareObjDict, which never populate bounding_box_px for such objects).
  """

  def test_pixel_bbox_path_still_used_when_available(self, mock_vdms_db):
    """Verify objects with a real pixel bbox still use the pixel-area gate, unchanged."""

    manager = UUIDManager()

    obj = MagicMock()
    obj.rv_id = "track_with_pixels"
    obj.reid = {"embedding_vector": np.arange(64, dtype=np.float32).tolist()}
    obj.boundingBoxPixels = MagicMock()
    obj.boundingBoxPixels.area = 10000  # above default minimum_bbox_area (5000)
    obj.size = None  # should be irrelevant since pixel bbox takes priority

    manager.gatherQualityVisualFeatures(obj)

    assert "track_with_pixels" in manager.quality_features, \
      "Should accept embedding via pixel-area gate when boundingBoxPixels is present"

  def test_pixel_bbox_path_still_rejects_when_area_too_small(self, mock_vdms_db):
    """Verify pixel bbox path still rejects embeddings below minimum_bbox_area."""

    manager = UUIDManager()

    obj = MagicMock()
    obj.rv_id = "track_small_pixels"
    obj.reid = {"embedding_vector": np.arange(64, dtype=np.float32).tolist()}
    obj.boundingBoxPixels = MagicMock()
    obj.boundingBoxPixels.area = 100  # below default minimum_bbox_area (5000)
    obj.size = [10.0, 10.0, 10.0]  # large metric size, should NOT be used as fallback here

    manager.gatherQualityVisualFeatures(obj)

    assert "track_small_pixels" not in manager.quality_features, \
      "Should reject via pixel-area gate even though metric size would pass"

  def test_falls_back_to_metric_area_when_no_pixel_bbox(self, mock_vdms_db):
    """Verify objects without boundingBoxPixels use the metric-space footprint area instead."""

    manager = UUIDManager()

    obj = Mock(spec=['rv_id', 'reid', 'size'])
    obj.rv_id = "track_child_forwarded"
    obj.reid = {"embedding_vector": np.arange(64, dtype=np.float32).tolist()}
    obj.size = [0.5, 0.5, 1.7]  # 0.85 m^2, above default minimum_bbox_area_m2 (0.15)

    manager.gatherQualityVisualFeatures(obj)

    assert "track_child_forwarded" in manager.quality_features, \
      "Should accept embedding via metric-area fallback when no pixel bbox is present"

  def test_metric_fallback_rejects_when_area_too_small(self, mock_vdms_db):
    """Verify embeddings are rejected when metric footprint area is below minimum_bbox_area_m2."""

    manager = UUIDManager()

    obj = Mock(spec=['rv_id', 'reid', 'size'])
    obj.rv_id = "track_tiny_footprint"
    obj.reid = {"embedding_vector": np.arange(64, dtype=np.float32).tolist()}
    obj.size = [0.05, 0.05, 0.05]  # 0.0025 m^2, below default minimum_bbox_area_m2 (0.15)

    manager.gatherQualityVisualFeatures(obj)

    assert "track_tiny_footprint" not in manager.quality_features, \
      "Should reject embedding when metric footprint area is too small"

  def test_metric_fallback_respects_custom_threshold_override(self, mock_vdms_db):
    """Verify an explicit minimum_bbox_area_m2 override is honored."""

    manager = UUIDManager()

    obj = Mock(spec=['rv_id', 'reid', 'size'])
    obj.rv_id = "track_custom_threshold"
    obj.reid = {"embedding_vector": np.arange(64, dtype=np.float32).tolist()}
    obj.size = [0.5, 0.5, 1.7]  # 0.85 m^2

    # Explicit override higher than the computed area should reject it
    manager.gatherQualityVisualFeatures(obj, minimum_bbox_area_m2=1.0)

    assert "track_custom_threshold" not in manager.quality_features, \
      "Should reject embedding when override threshold exceeds computed area"

  def test_discards_embedding_when_no_pixel_bbox_and_no_size(self, mock_vdms_db):
    """Verify embeddings are discarded (not silently accepted) when there is no quality signal at all."""

    manager = UUIDManager()

    obj = Mock(spec=['rv_id', 'reid'])
    obj.rv_id = "track_no_signal"
    obj.reid = {"embedding_vector": np.arange(64, dtype=np.float32).tolist()}
    # No boundingBoxPixels, no size attribute at all

    manager.gatherQualityVisualFeatures(obj)

    assert "track_no_signal" not in manager.quality_features, \
      "Should discard embedding when neither pixel bbox nor usable size is available"

  def test_discards_embedding_when_boundingboxpixels_explicitly_none(self, mock_vdms_db):
    """Verify boundingBoxPixels=None is treated the same as missing (falls back to metric gate)."""

    manager = UUIDManager()

    obj = Mock(spec=['rv_id', 'reid', 'boundingBoxPixels', 'size'])
    obj.rv_id = "track_explicit_none_bbox"
    obj.reid = {"embedding_vector": np.arange(64, dtype=np.float32).tolist()}
    obj.boundingBoxPixels = None
    obj.size = [0.5, 0.5, 1.7]  # 0.85 m^2, above default minimum_bbox_area_m2

    manager.gatherQualityVisualFeatures(obj)

    assert "track_explicit_none_bbox" in manager.quality_features, \
      "Should use metric fallback when boundingBoxPixels is explicitly None"

  def test_metric_fallback_respects_reid_enabled_flag(self, mock_vdms_db):
    """Verify no embedding is gathered at all when reid_enabled is False, even via metric fallback."""

    manager = UUIDManager()
    manager.reid_enabled = False

    obj = Mock(spec=['rv_id', 'reid', 'size'])
    obj.rv_id = "track_reid_disabled"
    obj.reid = {"embedding_vector": np.arange(64, dtype=np.float32).tolist()}
    obj.size = [0.5, 0.5, 1.7]

    manager.gatherQualityVisualFeatures(obj)

    assert "track_reid_disabled" not in manager.quality_features, \
      "Should not gather features when reid is disabled, regardless of bbox/size"
