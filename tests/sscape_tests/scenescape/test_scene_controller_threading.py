...
class TestSceneControllerDualMqttStyleConcurrency:
  """Parent MQTT ingest overlapping subscription refresh."""
  ...
  def test_parent_ingest_vs_subscription_refresh(
    self, _mock_epoch, _mock_adjust, _mock_metrics
  ):
    """External ingest and updateSubscriptions must not race catastrophically."""
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as pool:
      ...
      futures += [pool.submit(refresh) for _ in range(4)]
      for fut in concurrent.futures.as_completed(futures):
        fut.result()

    assert not errors, f"ingest vs subscription race: {errors}"
