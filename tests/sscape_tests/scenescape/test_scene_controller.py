...
class TestSceneControllerPublishers:
  ...
  def test_publish_external_detections_skips_standalone_root_scene(self):
    """Scenes without a parent skip hierarchy publish when publish-external is off (ADR 16)."""
    scene_controller = self._build_controller('unregulated', publish_external=False)
    scene = SimpleNamespace(
      uid='scene-1',
      parent=None,
      external_update_rate=2,
  ...
  def test_publish_external_detections_exports_root_when_flag_set(self):
    """Remote children opt in via CONTROLLER_PUBLISH_EXTERNAL / --publish-external."""
