...
GStreamer's ``add-reference-timestamp-meta`` steps the offset on every SR;
this element keeps the advertised clock-rate as the slope and servos only the
offset toward each report, so capture time stays aligned to the sender clock
without jittering the timeline at every report.
"""
