...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const generatePipelineUrl = document.getElementById('generate-pipeline-url')?.value;
    ...
    if (generateButton && pipelineField && generatePipelineUrl) {
        console.log('Pipeline generator: All elements found, attaching event listener...');
        ...
        generateButton.addEventListener('click', function(e) {
            ...
            console.log('Pipeline generator: Making fetch request to:', generatePipelineUrl);
            
            // Make AJAX request to generate pipeline
            ...
            .then(response => {
                console.log('Pipeline generator: Response received:', response.status);
            ...
            .catch(error => {
                ...
                alert('Error generating pipeline. Please check the console for details.');
            })
            .finally(() => {
                // Re-enable button and restore original text
                generateButton.disabled = false;
                generateButton.innerHTML = 'Generate Pipeline Preview';
                console.log('Pipeline generator: Request completed, button re-enabled');
            });
        });
    } else {
        console.log('Pipeline generator: Required elements not found');
        if (!generateButton) console.log('Pipeline generator: generate-pipeline-preview button not found');
        if (!pipelineField) console.log('Pipeline generator: id_camera_pipeline field not found');
        if (!generatePipelineUrl) console.log('Pipeline generator: generate-pipeline-url hidden input not found');
    }
});