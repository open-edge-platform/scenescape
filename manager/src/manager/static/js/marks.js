...
function addNewMark(
  ...
  // Create the core dot, plus a pulsing halo for moving/tracked objects.
  // Person/vehicle cores use a 4-quadrant checkerboard (black/color, like a
  // pinwheel roundel) so the black quadrants stay visible on light
  // backgrounds and the color quadrants stay visible on dark ones, no matter
  // what the UUID color happens to be.
  var halo;
  var circle;
  if (o.type === "apriltag") {
    circle = mark.circle(0, 0, mark_radius).addClass("mark-core");
    circle.attr("stroke", trackColor);
  } else {
    halo = mark
      .circle(0, 0, Math.round(mark_radius * 1.3))
      .addClass("mark-halo");
    halo.attr("fill", trackColor);
